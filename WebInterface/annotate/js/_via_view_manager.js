...
_via_view_manager.prototype._init_ui_elements = function () {
  ...
  this.pname = document.createElement("select");
  this.pname.setAttribute("class", "view_selector");
  this.pname.setAttribute("id", "via_client_selector");
  this.pname.setAttribute("title", "Select a client");

  this.pname.addEventListener(
    "change",
    this._on_client_selector_change.bind(this)
  ); 

  var pname_obj = this.pname;
  ...
  $.ajax({
    ...
    success: function (datas) {
      var opts = JSON.parse(datas);
      for (var d of opts) {
        var oi = document.createElement("option");
        oi.setAttribute("value", d.CLIENT_ID);
        oi.innerHTML = d.CLIENT_NAME;
        pname_obj.appendChild(oi);
      }
    },
    error: function () {
      console.log(error)
  ...
  this.view_selector.setAttribute("title", "Select a Camera");
  this.view_selector.setAttribute("style", "display:none;");
    this.view_selector.addEventListener(
    "change",
    this._on_view_selector_change.bind(this)
  );

  this.camera_selector = document.createElement("select");
  this.camera_selector.setAttribute("class", "view_selector");
  this.camera_selector.setAttribute("title", "Select a Camera");
  this.camera_selector.setAttribute("id", "via_camera_selector");
  this.view_selector.addEventListener(
    "change",
    this._on_camera_selector_change.bind(this)
  );  
  ...
  var client_id = this.pname.value;
  var camera_selector = this.camera_selector;
 
  $.ajax({
    url: "./get_cameras.php?client_id="+client_id,
    type: "get",
    async: false,     
    success: function (datas) {
      var opts = JSON.parse(datas);
      for (var d of opts) {
        var oi = document.createElement("option");
        oi.setAttribute("value", d.CAM_ID_LOCAL);
        oi.innerHTML = d.CAM_ID_REMOTE;
        camera_selector.appendChild(oi);
      }
    },
    error: function () {
      console.log(error)
  ...
  this.image_id_label = document.createElement("span");
  this.image_id_label.setAttribute("id", "image_id_label");
  this.image_id_label.setAttribute("style", "color:blue;");
  ...
  // this.view_filter_regex.addEventListener(
  //   "input",
  //   this._on_view_filter_regex_change.bind(this)
  // );

  this.c.innerHTML = "";
  this.c.appendChild(this.pname);
  this.c.appendChild(this.view_selector);
  this.c.appendChild(this.camera_selector);
  this.c.appendChild(this.image_id_label);

  this._init_camera_selector(client_id)
...
_via_view_manager.prototype._on_client_selector_change = function (e) {
  var client_id = e.target.options[e.target.selectedIndex].value;
  this._init_camera_selector(client_id);
  this.via.d.store.file = {};
};

_via_view_manager.prototype._on_camera_selector_change = function (e) {
  // var client_id = e.target.options[e.target.selectedIndex].value;
  // this._init_camera_selector(client_id);
  this.via.d.store.file = {};
};
...
_via_view_manager.prototype._init_camera_selector = function(client_id) {
  
  var camera_selector = document.getElementById('via_camera_selector');
  ...
  console.log(camera_selector.value)
}

_via_view_manager.prototype._on_next_view = function () {
  if (this.view_selector.options.length) {
    var vid = this.view_selector.options[this.view_selector.selectedIndex]
      .value;
...
_via_view_manager.prototype._on_event_project_updated = function (
  data,
  event_payload
) {
  ...
  if (regex === "" || typeof regex === "undefined") {
    ...
    if (this.view_selector.options.length) {
      if (this.view_selector.selectedIndex !== -1) {
        existing_vid = this.view_selector.options[
          this.view_selector.selectedIndex
        ].value;
      }
    }
