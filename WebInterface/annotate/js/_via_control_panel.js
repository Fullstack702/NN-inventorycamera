...
_via_control_panel.prototype._init = function(type) {
  ...
  // logo_panel.innerHTML =
  //   '<a href="http://www.robots.ox.ac.uk/~vgg/software/via/" title="VGG Image Annotator (VIA)" target="_blank">VIA</a>';
  ...
  var editor = _via_util_get_svg_button(
    "micon_insertcomment",
  ...
  var keyboard = _via_util_get_svg_button(
    "micon_keyboard",
    "Keyboard Shortcuts"
  );
...
_via_control_panel.prototype._add_view_manager_tools = function() {
  ...
  var ignore_btn = document.createElement("input");
  ignore_btn.setAttribute('type', 'button');
  ignore_btn.setAttribute('id', 'menu_ignore_btn');
  ignore_btn.setAttribute('value', 'Ignore');
  ignore_btn.setAttribute('onclick', 'set_ignore();');
  ignore_btn.setAttribute('style', 'font-size: 13px; cursor:pointer; padding:5px; border-radius:5px; background: black; color: white;  font-weight: bold;');
  this.c.appendChild(ignore_btn);

  var prev_btn = document.createElement("input");
  prev_btn.setAttribute('type', 'button');
  prev_btn.setAttribute('value', '<');
  prev_btn.setAttribute('onclick', 'set_navigate_image(-1);');
  prev_btn.setAttribute('style', 'width: 30px;  height: 30px; line-height:0.5;  font-size: 30px; cursor:pointer; border:0px;');
  this.c.appendChild(prev_btn);

  var next_btn = document.createElement("input");
  next_btn.setAttribute('type', 'button');
  next_btn.setAttribute('value', '>');
  next_btn.setAttribute('onclick', 'set_navigate_image(1);');
  next_btn.setAttribute('style', 'width: 30px;  height: 30px; line-height:0.5; font-size: 30px; cursor:pointer; border:0px;');
  this.c.appendChild(next_btn);

/* 
  var add_media_local = _via_util_get_svg_button(
    "micon_add_circle",
    "Add Audio or Video File in Local Computer",
    "add_media_local"
  );
  ...
  add_media_bulk.addEventListener(
    "click",
    this.via.vm._on_add_media_bulk.bind(this.via.vm)
  );
  add_media_bulk.addEventListener(
    "click",
    function() {
      var action_map = {
        via_page_fileuri_button_bulk_add: this._page_on_action_fileuri_bulk_add.bind(
          this
        )
      };
...
_via_control_panel.prototype._add_region_shape_selector = function() {
  ...
  var rect = _via_util_get_svg_button(
    "shape_rectangle",
    "Rectangle",
    "RECTANGLE"
  );
  ...
  var extreme_rect = _via_util_get_svg_button(
    ...
    "Extreme rectangle is defined using four points along the boundary of a rectangular object.",
    "EXTREME_RECTANGLE"
  );
  ...
  var extreme_circle = _via_util_get_svg_button(
    ...
    "Extreme circle is defined using any three points along the circumference of a circular object.",
    "EXTREME_CIRCLE"
  );
  ...
  var polyline = _via_util_get_svg_button(
    "shape_polyline",
    "Polyline",
    "POLYLINE"
  );
...
_via_control_panel.prototype._project_load_on_local_file_read = function(
  project_data_str
) {
  this.via.d.project_load(project_data_str);
};

_via_control_panel.prototype._project_import_via2_on_local_file_read = function(
  project_data_str
) {
...
_via_control_panel.prototype._add_project_share_tools = function() {
  if (this.via.s) {
    var imagebrowser = _via_util_get_svg_button(
      "image_browser",
      "Toggle Image Browser"
    );
    imagebrowser.addEventListener(
      "click",
      function() {
    ...
    var push = _via_util_get_svg_button(
      "micon_upload",
      "Push (i.e. share this project or share your updates made to this project)"
    );
    ...
    var pull = _via_util_get_svg_button(
      "micon_download",
      "Pull (i.e. open a shared project or fetch updates for the current project)"
    );
...
_via_control_panel.prototype.fileuri_bulk_add_image_from_file = function(
  uri_list_str
) {
  this.fileuri_bulk_add_from_url_list(uri_list_str, _VIA_FILE_TYPE.IMAGE);
};

_via_control_panel.prototype.fileuri_bulk_add_audio_from_file = function(
  uri_list_str
) {
  this.fileuri_bulk_add_from_url_list(uri_list_str, _VIA_FILE_TYPE.AUDIO);
};

_via_control_panel.prototype.fileuri_bulk_add_video_from_file = function(
  uri_list_str
) {
  this.fileuri_bulk_add_from_url_list(uri_list_str, _VIA_FILE_TYPE.VIDEO);
};

_via_control_panel.prototype.fileuri_bulk_add_auto_from_file = function(
  uri_list_str
) {
  this.fileuri_bulk_add_from_url_list(uri_list_str, 0);
};

_via_control_panel.prototype.fileuri_bulk_add_from_url_list = function(
  uri_list_str,
  type
) {
