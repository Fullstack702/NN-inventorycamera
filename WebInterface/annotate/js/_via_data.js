...
_via_data.prototype.attribute_update_anchor_id = function (aid, anchor_id) {
  return new Promise(function (ok_callback, err_callback) {
    ...
    this._cache_update_attribute_group();
    this.emit_event('attribute_update', {
      'aid': aid
    });
    ok_callback(aid);
  }.bind(this));
  ...
  return new Promise(function (ok_callback, err_callback) {
    try {
      ...
      if (typeof (emit) !== 'undefined'
        && emit === true) {
        this.emit_event('metadata_add_bulk', {
          'mid_list': added_mid_list
        });
      }
      ok_callback({
        'mid_list': added_mid_list
...
_via_data.prototype.metadata_update = function (vid, mid, z, xy, v) {
  return new Promise(function (ok_callback, err_callback) {
    try {
      ...
      var xy_fp = _via_util_float_arr_to_fixed(xy, _VIA_FLOAT_FIXED_POINT);
      this.store.metadata[mid] = new _via_metadata(vid, z_fp, xy_fp, av);
      this.emit_event('metadata_update', {
        'vid': vid,
        'mid': mid
      });
      ok_callback({
...
_via_data.prototype.metadata_delete = function (vid, mid) {
  return new Promise(function (ok_callback, err_callback) {
    try {
      ...
      this.emit_event('metadata_delete', {
        'vid': vid,
        'mid': mid
      });
      ok_callback({
...
_via_data.prototype.view_bulk_add_from_filelist = function (filelist) {
  return new Promise(function (ok_callback, err_callback) {
    try {
      ...
      var payload = {
        'vid_list': added_vid_list,
        'fid_list': added_fid_list
      };
...
//        filename.push(this.store.project.pid.substr(0, 8) + '_');
        filename.push(this.store.project.pid + '_');
      }
...
_via_data.prototype.save_data = function () {
  var metadata = this.store.metadata;
  var empty_flag = false;
  var av_check = true;
  Object.keys(metadata).forEach(function (k) {
 
    if (k != '' && isEmpty(metadata[k].av)) {
      av_check = false;
      return true;
    }
  });
  ...
  if (av_check)
    $.post("./save_data.php", JSON.stringify(this.store),

      function (data, status) {
        if(data  == "ok")
        _via_util_msg_show('Image Saved!');
        //    alert("Data: " + data + "\nStatus: " + status);
      });
  else {
    _via_util_msg_show("Please select the object!");
  }
}
...
_via_data.prototype.project_load_json = function (project_json_data) {
  return new Promise(function (ok_callback, err_callback) {
    try {
      ...
      // 
      var file_obj = project_data.file[Object.keys(project_data.file)[0]];
      if(file_obj.pic_id == null){
        _via_util_msg_show('Failed to load project! [ There is no picture.]');
        this._init_default_project();
...
_via_data.prototype.project_merge_on_success = function () {
  this._cache_update();
  this.emit_event('project_updated', {
    'pid': this.store.project.pid
  });
